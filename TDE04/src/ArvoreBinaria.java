...
class ArvoreBinaria {
    ...
    Node insereElemento(int dado) {
        ...
        while ((pai.esquerda != null && dado < pai.dado) || (pai.direita != null && dado > pai.dado)) {
            if (dado < pai.dado) pai = pai.esquerda;
            else pai = pai.direita;
        }
        if (dado < pai.dado) return pai.esquerda = novo;
        else return pai.direita = novo;
    }
    ...
    Node removeElemento(int dado) {
        ...
        while (raiz_atual != null) {
            if (dado == raiz_atual.dado) break;
            pai = raiz_atual;
            if (dado < raiz_atual.dado) raiz_atual = raiz_atual.esquerda;
            else raiz_atual = raiz_atual.direita;
        }

        if (raiz_atual == null) return null; // Não existe


        if (pai.direita == raiz_atual) pai.direita = null;
        else pai.esquerda = null;
